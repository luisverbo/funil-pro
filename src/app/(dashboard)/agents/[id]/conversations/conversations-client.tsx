...
import { listConversations, getConversation } from '@/app/actions/ai-agents'
            ...
            <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-2 bg-gray-50">
              {messages.map(m => (
                <div key={m.id} className={`max-w-[80%] px-3 py-2 rounded-2xl text-sm ${
                  m.role === 'lead' ? 'self-end bg-indigo-600 text-white rounded-br-sm' : 'self-start bg-white border text-gray-800 rounded-bl-sm'
                }`}>
                  {m.content}
                </div>
              ))}
