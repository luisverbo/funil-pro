    ...
    <div className="max-w-6xl mx-auto">
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-24 border-2 border-dashed border-gray-200 rounded-2xl bg-white">
          ...
          <button onClick={handleOpen} className="inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700 transition-colors">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filtered.map((page) => {
            const typeMeta = PAGE_TYPES.find((t) => t.type === page.page_type)
            return (
              <div key={page.id} className="bg-white rounded-2xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow overflow-hidden flex flex-col">
                ...
                <div className="px-4 pb-4 flex items-center gap-2">
                  <button onClick={() => router.push(getEditorPath(page))}
                  ...
                  <button onClick={() => handleDuplicate(page.id)}
                    className="px-3 py-2 border border-gray-200 text-gray-600 text-sm font-medium rounded-lg hover:bg-gray-50 transition-colors" title="Duplicar">
                  ...
                  {page.published && page.slug && (
                    <a href={`/pg/${page.slug}`} target="_blank" rel="noopener noreferrer"
                      className="px-3 py-2 border border-gray-200 text-gray-600 text-sm font-medium rounded-lg hover:bg-gray-50 transition-colors" title="Ver página">
                  ...
                  <button onClick={() => handleDelete(page.id, page.title)}
                    className="px-3 py-2 border border-red-100 text-red-400 text-sm font-medium rounded-lg hover:bg-red-50 transition-colors" title="Excluir">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
            <div className="flex items-center justify-between p-6 border-b border-gray-100">
              <div>
                ...
                <p className="text-sm text-gray-500">Passo {step} de {isQuiz ? '2' : '3'}</p>
              </div>
            ...
            {step === 1 && (
              <div className="p-6">
                ...
                <button onClick={() => setStep(2)} className="mt-6 w-full py-3 bg-indigo-600 text-white font-medium rounded-xl hover:bg-indigo-700">Continuar →</button>
              </div>
            )}
            ...
            {step === 2 && isQuiz && (
              <div className="p-6">
                ...
                <input
                  ...
                  autoFocus
                />
                <p className="text-xs text-gray-400 mt-2">Você será redirecionado para o editor visual de quiz.</p>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {TEMPLATE_OPTIONS.map(({ id, label, description, icon }) => (
                    <button key={id} onClick={() => setSelectedTemplate(id)}
                      className={`p-4 rounded-xl border-2 text-left transition-all ${
                        selectedTemplate === id ? 'border-indigo-500 bg-indigo-50' : 'border-gray-200 hover:border-gray-300'
                      }`}>
                      <span className="text-2xl">{icon}</span>
            ...
            {step === 3 && !isQuiz && (
              <div className="p-6">
                ...
                <input
                  className="w-full border border-gray-300 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400"
                  placeholder="Ex: Página de Captura - Produto X"
                  value={pageName}
                  onChange={e => setPageName(e.target.value)}
                  onKeyDown={e => e.key === 'Enter' && handleCreate()}
                  autoFocus
