...
import { captureTemplate, vslTemplate, deliveryTemplate, thankYouTemplate, salesTemplate } from '@/lib/page-templates'
...
const TEMPLATE_OPTIONS = [
  ...
  { id: 'thankyou', label: 'Obrigado Simples', description: 'Check animado + CTA', icon: '🙏' },
  { id: 'sales', label: 'Carta de Vendas', description: 'Página longa completa', icon: '💰' },
]
...
export default function PagesClient({ pages, tenantId }: { pages: any[]; tenantId: string }) {
  ...
  const handleOpen = () => { setStep(1); setSelectedType('capture'); setSelectedTemplate('blank'); setPageName(''); setPageSlug(''); setShowModal(true) }

  const handleNameChange = (name: string) => {
    setPageName(name)
    if (!pageSlug || pageSlug === generateSlug(pageName).split('-').slice(0, -1).join('-') + '-') {
  ...
  const handleDuplicate = (id: string) => {
    startTransition(async () => { const copy = await duplicatePage(id); router.push(`/page-editor/${copy.id}`) })
  }
            ...
            <div key={page.id} className="bg-white rounded-2xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow overflow-hidden flex flex-col">
              ...
              <div className="p-4 flex-1 flex flex-col gap-1">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <span className={`text-xs font-medium px-2 py-0.5 rounded-full shrink-0 ${TYPE_COLORS[page.page_type] ?? 'bg-gray-100 text-gray-600'}`}>{PAGE_TYPES.find((t) => t.type === page.page_type)?.label ?? page.page_type}</span>
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
            <div className="flex items-center justify-between p-6 border-b border-gray-100">
              <div><h2 className="text-xl font-bold text-gray-900">Nova Página</h2><p className="text-sm text-gray-500">Passo {step} de 3</p></div>
            ...
            {step === 3 && (
              <div className="p-6">
                ...
                <div className="flex gap-3 mt-6">
                  ...
                  <button onClick={handleCreate} disabled={!pageName.trim() || isPending} className="flex-1 py-3 bg-indigo-600 text-white font-medium rounded-xl hover:bg-indigo-700 disabled:opacity-50 transition-colors">{isPending ? 'Criando...' : 'Criar Página'}</button>
                </div>
              </div>
            )}
          </div>
