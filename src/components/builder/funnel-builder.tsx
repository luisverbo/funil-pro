...
function BuilderCanvas({ funnel, initialBlocks, initialEdges, blockMetrics, waInstances = [] }: Props) {
  ...
  const handleAutoLayout = useCallback(() => {
    ...
    for (const n of nodes) {
      if (!positions[n.id]) { positions[n.id] = { x: 0, y: extraRow * 160 }; extraRow++ }
    }
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="h-14 bg-white border-b border-gray-200 flex items-center justify-between px-4 shrink-0 gap-2">
          <div className="flex items-center gap-2 min-w-0 flex-1">
            ...
            {editingName ? (
              <input autoFocus value={funnelName} onChange={(e) => setFunnelName(e.target.value)} onBlur={() => setEditingName(false)} onKeyDown={(e) => e.key === 'Enter' && setEditingName(false)} className="text-sm font-semibold text-gray-800 border-b-2 border-indigo-500 bg-transparent outline-none px-0.5 min-w-0 w-40" />
            ) : (
              <button onClick={() => setEditingName(true)} className="text-sm font-semibold text-gray-800 hover:text-indigo-600 transition-colors truncate max-w-[160px]">{funnelName}</button>
            )}
            <span className="text-xs font-semibold px-2 py-0.5 rounded-full shrink-0 hidden sm:inline-flex" style={{ color: statusMeta.color, backgroundColor: statusMeta.bg }}>{statusMeta.label}</span>
            <TriggerSelector funnelId={funnel.id} initialTriggers={((funnel as Props['funnel']).funnel_product_triggers ?? []) as unknown as { id: string; platform: string; product_name: string; trigger_event: 'purchase' | 'abandoned_cart' }[]} />
            <div className="relative shrink-0 hidden sm:block">
              <button onClick={() => setWaDropdownOpen((v) => !v)} className={`flex items-center gap-1.5 text-xs px-2.5 py-1.5 rounded-lg border transition-colors font-medium ${ selectedInstance ? selectedInstance.status === 'connected' ? 'border-green-200 bg-green-50 text-green-700' : 'border-red-200 bg-red-50 text-red-600' : 'border-amber-200 bg-amber-50 text-amber-600' }`}>
                {selectedInstance ? (<><span className={`w-1.5 h-1.5 rounded-full shrink-0 ${selectedInstance.status === 'connected' ? 'bg-green-500' : 'bg-red-400'}`} /><span className="max-w-[80px] truncate">{selectedInstance.instance_name}</span></>) : (<span>Sem WA</span>)}
              ...
              {waDropdownOpen && (
                <div className="absolute top-full left-0 mt-1 w-56 bg-white border border-gray-200 rounded-xl shadow-lg z-50 overflow-hidden">
                  <button onClick={() => handleSelectWaInstance(null)} className={`w-full text-left px-3 py-2 text-xs hover:bg-gray-50 transition-colors flex items-center gap-2 ${!waInstanceId ? 'bg-gray-50 font-medium' : ''}`}><span className="w-1.5 h-1.5 rounded-full bg-gray-300 shrink-0" />Nenhum</button>
                  {waInstances.map((inst) => (<button key={inst.id} onClick={() => handleSelectWaInstance(inst.id)} className={`w-full text-left px-3 py-2 text-xs hover:bg-gray-50 transition-colors flex items-center gap-2 ${waInstanceId === inst.id ? 'bg-indigo-50' : ''}`}><span className={`w-1.5 h-1.5 rounded-full shrink-0 ${inst.status === 'connected' ? 'bg-green-500' : inst.status === 'connecting' ? 'bg-amber-400' : 'bg-red-400'}`} /><span className="flex-1 truncate font-medium text-gray-700">{inst.instance_name}</span></button>))}
                </div>
              )}
            </div>
          </div>
          <div className="flex items-center gap-1.5 shrink-0">
            {statusMsg && <span className={`text-xs font-medium hidden sm:block ${statusMsg.ok ? 'text-emerald-600' : 'text-red-500'}`}>{statusMsg.text}</span>}
            <button onClick={undo} disabled={!canUndo} title="Desfazer (Ctrl+Z)" className="p-1.5 rounded-lg text-gray-500 hover:bg-gray-100 disabled:opacity-30 transition-colors"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="w-4 h-4"><path d="M3 7v6h6" /><path d="M21 17a9 9 0 0 0-9-9 9 9 0 0 0-6 2.3L3 13" /></svg></button>
            <button onClick={redo} disabled={!canRedo} title="Refazer (Ctrl+Y)" className="p-1.5 rounded-lg text-gray-500 hover:bg-gray-100 disabled:opacity-30 transition-colors"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="w-4 h-4"><path d="M21 7v6h-6" /><path d="M3 17a9 9 0 0 1 9-9 9 9 0 0 1 6 2.3L21 13" /></svg></button>
            <button onClick={handleAutoLayout} title="Organizar automaticamente" className="hidden sm:flex items-center gap-1 text-xs px-2.5 py-1.5 rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50 transition-colors font-medium"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="w-3.5 h-3.5"><rect x="3" y="3" width="7" height="7" rx="1" /><rect x="14" y="3" width="7" height="7" rx="1" /><rect x="3" y="14" width="7" height="7" rx="1" /><rect x="14" y="14" width="7" height="7" rx="1" /></svg>Organizar</button>
            <button onClick={() => { setShowVersionDrawer((v) => !v); setShowCaptureEditor(false); setShowLinksDrawer(false); setSelectedNodeId(null) }} className="hidden sm:flex items-center gap-1 text-xs px-2.5 py-1.5 rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50 transition-colors font-medium"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="w-3.5 h-3.5"><path d="M3 3v5h5" /><path d="M3.05 13A9 9 0 1 0 6 5.3L3 8" /><polyline points="12,7 12,12 15,15" /></svg>Versões</button>
            <button onClick={() => { setShowLinksDrawer(v => !v); setShowCaptureEditor(false); setShowVersionDrawer(false) }} className="hidden sm:flex items-center gap-1 text-xs px-2.5 py-1.5 rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50 transition-colors font-medium"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="w-3.5 h-3.5"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71" /><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71" /></svg>Links</button>
            <button onClick={() => { setShowCaptureEditor(v => !v); setShowLinksDrawer(false); setShowVersionDrawer(false) }} className="hidden sm:flex items-center gap-1 text-xs px-2.5 py-1.5 rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50 transition-colors font-medium"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="w-3.5 h-3.5"><circle cx="12" cy="12" r="10" /><line x1="2" y1="12" x2="22" y2="12" /><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z" /></svg>Página</button>
            <button onClick={handleSave} disabled={isPending} className="flex items-center gap-1 text-xs px-2.5 py-1.5 rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50 disabled:opacity-50 transition-colors font-medium">{isPending ? <svg className="animate-spin w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path d="M21 12a9 9 0 1 1-6.219-8.56" /></svg> : <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="w-3.5 h-3.5"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z" /><polyline points="17,21 17,13 7,13 7,21" /><polyline points="7,3 7,8 15,8" /></svg>}Salvar</button>
            <button onClick={handlePublish} disabled={isPending || funnelStatus === 'published'} className="flex items-center gap-1 text-xs px-2.5 py-1.5 rounded-lg bg-indigo-600 text-white hover:bg-indigo-700 disabled:opacity-50 transition-colors font-medium"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="w-3.5 h-3.5"><path d="M4.5 16.5c-1.5 1.26-2 5-2 5s3.74-.5 5-2c.71-.84.7-2.13-.09-2.91a2.18 2.18 0 0 0-2.91-.09z" /><path d="m12 15-3-3a22 22 0 0 1 2-3.95A12.88 12.88 0 0 1 22 2c0 2.72-.78 7.5-6 11a22.35 22.35 0 0 1-4 2z" /><path d="M9 12H4s.55-3.03 2-4c1.62-1.08 5 0 5 0" /><path d="M12 15v5s3.03-.55 4-2c1.08-1.62 0-5 0-5" /></svg>{funnelStatus === 'published' ? 'Publicado' : 'Publicar'}</button>
          </div>
        </header>
        ...
        <div className="flex-1 flex overflow-hidden">
          <div className="flex-1 relative">
            <ReactFlow
              ...
              deleteKeyCode="Delete" selectionOnDrag multiSelectionKeyCode="Shift"
          ...
          {selectedNodeId && !showCaptureEditor && !showLinksDrawer && !showVersionDrawer && (
            <ConfigPanel selectedNodeId={selectedNodeId} nodes={nodes} onClose={() => setSelectedNodeId(null)} funnelId={funnel.id} onOpenCaptureEditor={() => setShowCaptureEditor(true)} waInstances={waInstances} waInstanceId={waInstanceId} />
          )}
          {showCaptureEditor && (
            <CapturePageEditor funnelId={funnel.id} onClose={() => setShowCaptureEditor(false)} entryType={(nodes.find(n => n.type === 'entry')?.data as FunnelNodeData | undefined)?.config?.entry_type as string | undefined} onSaved={(tpl) => { setNodes((nds) => nds.map((n) => { if (n.type !== 'entry') return n; const nd = n.data as FunnelNodeData; return { ...n, data: { ...nd, config: { ...nd.config, page_configured: true, page_template: tpl } } } })) }} />
          )}
          {showLinksDrawer && <LinksDrawer funnelId={funnel.id} onClose={() => setShowLinksDrawer(false)} entryType={(nodes.find(n => n.type === 'entry')?.data as FunnelNodeData | undefined)?.config?.entry_type as string | undefined} />}
          {showVersionDrawer && <VersionDrawer funnelId={funnel.id} currentSnapshot={getSnapshot()} onRestore={handleRestoreVersion} onClose={() => setShowVersionDrawer(false)} />}
        </div>
      </div>

      <div className="md:hidden">
        <button onClick={() => setShowMobilePalette(true)} className="fixed bottom-6 right-6 z-30 w-14 h-14 bg-indigo-600 text-white rounded-full shadow-lg flex items-center justify-center text-2xl font-light hover:bg-indigo-700" aria-label="Adicionar bloco">+</button>
        {showMobilePalette && (
          <>
