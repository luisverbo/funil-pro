...
export default function DeleteFunnelButton({ funnelId, funnelName, isPublished, inline }: Props) {
    ...
    <>
      ...
      {open && createPortal(
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/30 backdrop-blur-sm" onClick={() => { if (!loading) setOpen(false) }} />
            ...
            {isPublished && (
              <div className="bg-amber-50 border border-amber-200 rounded-lg px-3 py-2.5 mb-4">
                <p className="text-xs text-amber-700 font-medium">
                  ⚠️ Este funil está publicado e pode ter leads em execução.
                </p>
              </div>
            )}
