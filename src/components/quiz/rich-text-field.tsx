...
/**
 * Editor de texto rico com toolbar flutuante (estilo Inlead).
 * Usa document.execCommand (nativo, sem dependência externa) — funciona em todos
 * os navegadores atuais. O HTML é salvo em `value` via onChange.
 */
export default function RichTextField({ value, onChange, placeholder }: Props) {
  ...
  // Inicializa/atualiza o conteúdo sem quebrar o cursor durante a digitação
  useEffect(() => {
  ...
  const updateToolbar = useCallback(() => {
    ...
    // Só mostra se a seleção está dentro do editor
    if (!ref.current.contains(range.commonAncestorContainer)) { setShowToolbar(false); return }
    const rect = range.getBoundingClientRect()
    const editorRect = ref.current.getBoundingClientRect()
    if (rect.width === 0 && rect.height === 0) {
      // cursor sem seleção — posiciona no topo do editor
      setToolbarPos({ top: -48, left: 0 })
    } else {
      setToolbarPos({ top: rect.top - editorRect.top - 48, left: Math.max(0, rect.left - editorRect.left) })
    }
    setShowToolbar(true)
  }, [])
  ...
  const Btn = ({ onClick, children, title }: { onClick: () => void; children: React.ReactNode; title: string }) => (
    <button type="button" title={title}
      ...
      className="w-7 h-7 flex items-center justify-center rounded hover:bg-gray-100 text-gray-700 text-sm">
      {children}
    </button>
  )
  ...
  return (
    <div className="relative">
      {showToolbar && (
        <div
          className="absolute z-30 flex items-center gap-0.5 bg-white border border-gray-200 rounded-lg shadow-lg px-1 py-1"
        ...
        >
          ...
          <select
            ...
            className="text-xs border border-gray-200 rounded px-1 h-7 bg-white"
            defaultValue=""
          >
