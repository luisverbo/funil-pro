...
// Envia lembrete (WhatsApp + e-mail) das reuniões que começam na próxima ~1h e
// ainda não foram lembradas. Chamado pelo cron de queue (VPS, todo minuto) e
// pela rota /api/agents/meetings/remind. Idempotente via reminder_sent_at.
export async function sendMeetingReminders(): Promise<{ sent: number; error?: string }> {
