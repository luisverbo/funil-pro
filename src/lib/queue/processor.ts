...
export async function processJob(job: QueueJob): Promise<void> {
  ...
  if (block.block_type === 'message') {
    ...
    const condition = matched ? 'yes' : 'no'
    await enqueueNext(job, condition, admin)
    if (!matched) await enqueueNext(job, 'default', admin)
  }

  else if (block.block_type === 'tag') {
    const tagAction = (config.tag_action as string) ?? 'add'
    ...
    if (tagName) {
      ...
      const newTags = tagAction === 'remove' ? tags.filter((t: string) => t !== tagName) : [...new Set([...tags, tagName])]
    ...
    const message = (config.message as string) ?? paymentLink
